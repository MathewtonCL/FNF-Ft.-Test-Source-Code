...
class TestStoryMenuState extends MusicBeatState
{
	...
	var grpWeekText:FlxTypedGroup<TestMenuItem>;
	var grpWeekCharacters:FlxTypedGroup<TestMenuCharacter>;
	...
	override function create()
	{
		...
		var yellowBG:FlxSprite = new FlxSprite(0, 56).makeGraphic(FlxG.width, 400, 0xFFA1A1A1);

		grpWeekText = new FlxTypedGroup<TestMenuItem>();
		add(grpWeekText);

		var blackBarThingie:FlxSprite = new FlxSprite().makeGraphic(FlxG.width, 56, FlxColor.BLACK);
		add(blackBarThingie);

		grpWeekCharacters = new FlxTypedGroup<TestMenuCharacter>();
		...
		{
			var weekThing:TestMenuItem = new TestMenuItem(0, yellowBG.y + yellowBG.height + 10, i);
		...
		for (char in 0...3)
		{
			var weekCharacterThing:TestMenuCharacter = new TestMenuCharacter((FlxG.width * 0.25) * (1 + char) - 150, weekCharacters[curWeek][char]);
	...
	function selectWeek()
	{
		if (weekUnlocked[curWeek])
		{
			...
			PlayState.isTestStoryMode = true;
