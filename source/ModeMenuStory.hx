...
typedef GameModeMenuStory = {
	var name:String;
	var characterName:String;
	//var portrait:String;
}

class ModeMenuStory extends MusicBeatState
{
	var menuItems:Array<String> = ['NORMALMODE', 'TESTMODE'];
	...
	public static var characterShit:Array<GameModeMenuStory>;
	...
	{
		menuBG = new FlxSprite().loadGraphic('assets/images/modeSelect/BGDesat.png');
		menuBG.updateHitbox();
		menuBG.screenCenter();
		...
		idleCharacter.frames = FlxAtlasFrames.fromSparrow('assets/images/modeSelect/selectmode.png', 'assets/images/modeSelect/selectmode.xml');
		idleCharacter.animation.addByPrefix('regmode', 'regmode instance 1', 24);
		idleCharacter.animation.addByPrefix('testmode', 'testmode instance 1', 24);
		idleCharacter.animation.addByPrefix('regmodeselect', 'regmodeselect instance 1', 24);
		idleCharacter.animation.addByPrefix('testmodeselect', 'testmodeselect instance 1', 24);
		
		idleCharacter.animation.play('regmode');
		...
		menuIcon.frames = FlxAtlasFrames.fromSparrow('assets/images/modeSelect/icons.png', 'assets/images/modeSelect/icons.xml');
		menuIcon.x = idleCharacter.x + 60;
		menuIcon.y = idleCharacter.y + 300;
		menuIcon.antialiasing = true;
		menuIcon.animation.addByPrefix('regmodeI', 'reg bf', 1);
		menuIcon.animation.addByPrefix('testmodeI', 'test bf', 1);
		add(menuIcon);
		...
		var charSelHeaderText:Alphabet = new Alphabet(0, 50, 'MODE SELECT', true, false);
		charSelHeaderText.screenCenter(X);
		add(charSelHeaderText);

		var selectionArrows:FlxSprite = new FlxSprite().loadGraphic('assets/images/modeSelect/arrowsz.png');
				...
				if (accepted)
				{
					...
					switch (daSelected)
					{
						case "NORMALMODE":
							...
							idleCharacter.animation.play('regmodeselect');
						...
						case "TESTMODE":
							FlxG.sound.play('assets/sounds/confirmMenu' + TitleState.soundExt);
							idleCharacter.animation.play('testmodeselect');
							idleCharacter.y -= 30;
							idleCharacter.x -= 10;
							FlxFlicker.flicker(grpMenuShit.members[curSelected],0);
						//	FlxG.sound.music.stop();
							//PlayState.bfMode = 'bf-test';
							//StoryMenuState.suffixThing = '-test';

					
							new FlxTimer().start(1, function(tmr:FlxTimer)
							{
								FlxG.switchState(new TestCharMenuStory());
	...
	function charCheckLmao()
	{
		...
		switch (daSelected)
		{
			case "NORMALMODE":
				idleCharacter.animation.play('regmode');
				idleCharacter.screenCenter(XY);
				menuBG.loadGraphic('assets/images/modeSelect/BGDesat.png');
				menuIcon.animation.play('regmodeI');
				menuBG.color = 0xFF3841BB;
			case "TESTMODE":
				idleCharacter.animation.play('testmode');
				idleCharacter.screenCenter(XY);
				menuBG.loadGraphic('assets/images/modeSelect/BGDesat.png');
				menuIcon.animation.play('testmodeI');
				menuBG.color = 0xFF87A3AD;
		}
